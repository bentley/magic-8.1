...
proc magic::maketoolbar {} {

   global Opts
   if {$Opts(toolbar) == 1} {return}	;# Already created

   toplevel .toolbar \
   	-visual "[winfo visual .layout1.magic] [winfo depth .layout1.magic]"
   ...
   set alltools [winfo children .toolbar]
   foreach i $alltools {
      destroy $i
   }
   ...
   set all_layers [concat {errors labels subcell} [magic::tech layer "*"]]
   foreach layername $all_layers {
      button .toolbar.b$layername -image img_$layername -command \
		"see $layername"

      # Bindings:  Entering the button puts the canonical layer name in the 
      # message window.
      bind .toolbar.b$layername <Enter> \
		[subst {focus %W ; .titlebar.message configure \
		 -text "$layername"}]
      bind .toolbar.b$layername <Leave> \
		[subst {.titlebar.message configure -text ""}]
      ...
      bind .toolbar.b$layername <ButtonPress-2> "paint $layername"
      bind .toolbar.b$layername <KeyPress-p> "paint $layername"
      bind .toolbar.b$layername <Shift-ButtonPress-2> "erase $layername"
      bind .toolbar.b$layername <KeyPress-e> "erase $layername"
      bind .toolbar.b$layername <ButtonPress-3> "see no $layername"
      bind .toolbar.b$layername <KeyPress-s> "select more area $layername"
      bind .toolbar.b$layername <KeyPress-S> "select less area $layername"
   }
   ...
   foreach layername $all_layers {
      button .toolbar.p$layername -image pale_$layername -command \
		"see $layername"
      bind .toolbar.p$layername <ButtonPress-3> \
		"see no $layername"
      bind .toolbar.p$layername <Enter> \
		[subst {focus %W ; .titlebar.message configure \
		 -text "$layername (locked)"}]
      bind .toolbar.p$layername <Leave> \
		[subst {.titlebar.message configure -text ""}]
   }
   ...
   foreach layername $all_layers {
      if {[lsearch $locklist $layername] >= 0} {
         grid .toolbar.p$layername -row $i -column 0 -sticky news
      } else {
         grid .toolbar.b$layername -row $i -column 0 -sticky news
      }
      bind .toolbar.p$layername <KeyPress-u> \
		"tech layer unlock $layername ; \
		grid forget .toolbar.p$layername ; \
		grid .toolbar.b$layername \
		-row $i -column 0 -sticky news"
      bind .toolbar.b$layername <KeyPress-l> \
		"tech layer lock $layername ; \
		grid forget .toolbar.b$layername ; \
		grid .toolbar.p$layername \
		-row $i -column 0 -sticky news"
      incr i
   }
   set Opts(toolbar) 1		;# Mark toolbar as being created
}
...
# #################################
   ...
   $m add check -label "Toolbar" -variable Winopts(${framename},toolbar) \
	-command [subst {if { \$Winopts(${framename},toolbar) } { \
		${framename}.toolbar clone .toolbar ; \
